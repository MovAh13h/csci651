...
# date -Ins >> node1.txt
echo "Starting 1"
docker run --cap-add=NET_ADMIN -p 8080:8080 --name n1 --net nodenet --ip 172.18.0.21 javaapptest 1 230.230.230.230 63001 >> node1.txt &
...
# date -Ins >> node2.txt
echo "Starting 2"
docker run --cap-add=NET_ADMIN -p 8081:8080 --name n2 --net nodenet --ip 172.18.0.22 javaapptest 2 230.230.230.230 63001 >> node2.txt &
...
# date -Ins >> node3.txt
echo "Starting 3"
docker run --cap-add=NET_ADMIN -p 8082:8080 --name n3 --net nodenet --ip 172.18.0.23 javaapptest 3 230.230.230.230 63001 >> node3.txt &
